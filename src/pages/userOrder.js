...
const UserOrder = (props) => {
    ...
    const getLocationbyUserIDApiData = (id) => {
        fetch(`${process.env.REACT_APP_EC2_HOST}/getLocation?id=` + (id)).then((resp) => resp.json()).then((d) => {
            // setDish(d);

            chefLoc = {lat: parseFloat(d?.data.lat), lng: parseFloat(d?.data.lan)};
            mapRun(map, google, userLoc, chefLoc);
            console.log(`${process.env.REACT_APP_EC2_HOST}/getLocation?id=` + (id), userLoc, chefLoc)
        }).catch((err) => {
            console.log(err);
        })
    }
    ...
    const getOrderbyIDApiData = () => {
        fetch(`${process.env.REACT_APP_EC2_HOST}/getOrderDetailsById?id=` + (order_id)).then((resp) => resp.json()).then((d) => {
            ...
            getLocationbyUserIDApiData(d.data?.[0].chef_id)
